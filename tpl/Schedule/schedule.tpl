...
{function name=displayReservable}
	<td {$spantype|default:'col'}span="{$Slot->PeriodSpan()}" ref="{$SlotRef}" class="reservable clickres slot" data-href="{$Href}" data-start="{$Slot->BeginDate()->Format('Y-m-d H:i:s')|escape:url}" data-end="{$Slot->EndDate()->Format('Y-m-d H:i:s')|escape:url}" data-resourceId="{$ResourceId}">&nbsp;</td>
{/function}
...
{block name="header"}
	{include file='globalheader.tpl' cssFiles='css/jquery.qtip.min.css,scripts/css/jqtree.css,css/schedule.css,scripts/css/colorbox.css' printCssFiles='css/schedule-print.css'}
{/block}
...
{block name="schedule_control"}
	<div>
		<div class="schedule_title">
			...
			<div id="individualDates"><label> Specific Dates <input type="checkbox" id="multidateselect"></label> <a href="#" id="individualDatesGo">Go</a></div>
